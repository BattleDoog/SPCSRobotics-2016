...
class ThreadedClient(object):
	...
	def avoidance(self, virtual_world):
		...
		while not self.gQuit:
			goalX = virtual_world.goal_x
			goalY = virtual_world.goal_y 
			goalVectorX = vrobot.x - goalX
			goalVectorY = vrobot.y - goalY
			theta = math.atan2(goalY, goalX) - math.pi / 2
			print theta, vrobot.a
			...
			if (virtual_world.go):
			    ...
			    if (p_l > 65 and p_r > 65): # free to move remember now the sensor data is distance in mm
			        vrobot.sl = 30  + int(2 * err +0.5)
			        vrobot.sr = 30 - int(2 * err +0.5)# moving forward
			    ...
			    if vrobot.x == goalX and vrobot.y == goalY:
			    	robot.set_wheel(0, 0)
			    	robot.set_wheel(1, 0)
			    moving = True
			else:
